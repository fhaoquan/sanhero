...
void CCmpPathfinder::ComputeShortPath2(ICmpObstructionManager* cmpObstructionManager, const IObstructionTestFilter& filter,
	entity_pos_t x0, entity_pos_t z0, entity_pos_t clearance,
	entity_pos_t range, const PathGoal& goal, pass_class_t passClass, WaypointPath& path)
{
	PROFILE("ComputeShortPath");
	entity_pos_t EDGE_EXPAND_DELTA2  = entity_pos_t::FromInt(1) / 16;
	m_DebugOverlayShortPathLines.clear();
	...
	for (size_t i = 0; i < squares.size(); ++i)
	{
		...
		CFixedVector2D hd0(squares[i].hw + pathfindClearance + EDGE_EXPAND_DELTA2, squares[i].hh + pathfindClearance + EDGE_EXPAND_DELTA2);
		CFixedVector2D hd1(squares[i].hw + pathfindClearance + EDGE_EXPAND_DELTA2, -(squares[i].hh + pathfindClearance + EDGE_EXPAND_DELTA2));
